		...
		<script>
			$(document).ready(function(){				
				...
				$("#approval").click(function() {
					var wDenyReason = "";
					$("#wDenyReason").val(wDenyReason);
					$("#insertForm").attr("action","/chainCo/workflow/doDenyApproval.do");
					$("#insertForm").submit();
				});
				$("#deny").click(function() {
					
					var wDenyReason = prompt("반려사유를 작성하십시오", "");
					$("#wDenyReason").val(wDenyReason);					
					$("#insertForm").attr("action","/chainCo/workflow/doDenyApproval.do");
					$("#insertForm").submit();
				});			
				...
				$("#goback").click(function(){
					$("#insertForm").attr("action","/chainCo/workflow/viewReceptionBox.do");
					$("#insertForm").submit();
				});
			});
		...
		<section>
			...
			<form id="insertForm" name="insertForm" method="POST">				 				 
				...
				<table id="table1">
					<tr>
						<td>제목</td>
						<td>${viewDetail.ePos}</td>
					</tr>
					<tr>
						<td>소속부서</td>
						<td>${viewDetail.eDept}</td>
					</tr>
					<tr>
						<td>직위</td>
						<td>${viewDetail.ePos}</td>
					...
					<tr>
						<td>작성자</td>
						<td>${viewDetail.eName}</td>
					</tr>
					<tr>
						<td>작성일자</td>
						<td>${viewDetail.wInsertDate}</td>
					</tr>						
					<tr>
						<td>금일실시사항</td>
					</tr>
					...
					<tr>
						<td>${viewDetail.wWtodayDone1}</td>
						<td>${viewDetail.wWtodayDone2}</td>
					</tr>					
					<tr>
						<td>익일예정사항</td>
						<td>${viewDetail.wWnextdayDo}</td>
					</tr>						
					<tr>
						<td>지시사항</td>
						<td>${viewDetail.wWinstruction}</td>
					</tr>
				</table>
				<input type="hidden" id="wDenyReason" name="wDenyReason">
			</form>
				<input type="button" class="go" id="approval" name="approval" value="승인">
				<input type="button" class="go" id="deny" name="deny" value="반려">
